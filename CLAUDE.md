...
| `docker-compose.yml` | Three profiles: `build` (host-arch), `runtime` (host-networking server with `./data` bind-mount only — no profile bind-mount; profile is rendered into `data/profile.yaml` on first start), `build-client` (Linux AppImage — pinned `linux/amd64` because the AppImage target is x86_64 by definition). Optional `ADAPTIX_TEAMSERVER_PASSWORD` / `ADAPTIX_OPERATORS` env vars pin credentials at first render. |
