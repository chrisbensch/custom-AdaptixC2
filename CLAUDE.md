...
| `docker/entrypoint.sh` | Runtime image's first-start bootstrap. Starts as root so it can `chown -R adaptix:adaptix /app/data` (the bind mount comes in with the host's ownership), generates self-signed TLS cert+key into `/app/data/` if missing, renders `/app/profile.yaml.tmpl` → `/app/data/profile.yaml` substituting `ADAPTIX_TEAMSERVER_PASSWORD` and `ADAPTIX_OPERATORS` (env vars or `openssl rand` defaults), and persists the resolved credentials to `/app/data/credentials.txt` (mode 600). Re-chowns `/app/data` once more after writes, then `exec gosu adaptix "$@"` to launch the server as UID 10001. Subsequent starts reuse the rendered profile — edit `data/profile.yaml` and restart, or `rm` it to re-render. See `BLUEPRINT.md` §5.7. |
