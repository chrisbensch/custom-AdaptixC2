...
| File | Purpose |
|---|---|
| `Dockerfile` | Multi-stage build, **host-arch by default** (verified arm64 + amd64): `base` (Go 1.25.4 + mingw-w64 + clang/nasm/llvm; `go-win7` pinned via `ARG GO_WIN7_SHA`) → `build-bofs` (Extension-Kit with the nanodump patch applied — 9 BOFs strict, SAL-BOF best-effort because of its network fetch — + PostEx-Arsenal) → `build-server` (server + 9 extenders, including Kharon grafted at build time) → `runtime` (debian-slim with `curl` for HEALTHCHECK; entrypoint COPYed from `docker/entrypoint.sh`; `EXPOSE 4321` only; `HEALTHCHECK` probes `https://127.0.0.1:4321/endpoint`). Force a specific arch with `DOCKER_DEFAULT_PLATFORM=linux/amd64`. |
| `docker-compose.yml` | Three profiles: `build` (host-arch), `runtime` (host-networking server with `./data` bind-mount only — no profile bind-mount; profile is rendered into `data/profile.yaml` on first start), `build-client` (Linux AppImage — multi-arch via `ADAPTIX_CLIENT_*` env vars, defaults to `linux/amd64` for backward compatibility). Optional `ADAPTIX_TEAMSERVER_PASSWORD` / `ADAPTIX_OPERATORS` env vars pin credentials at first render. |
| `profile.kharon.yaml` | The server profile **template** baked into the runtime image at `/app/profile.yaml.tmpl`. Diff vs. upstream `AdaptixC2/AdaptixServer/profile.yaml`: adds the two Kharon extenders and the two AxScript module sets (`Extension-Kit/extension-kit.axs`, `PostEx-Arsenal/kh_modules.axs`), replaces literal passwords with `__ADAPTIX_TEAMSERVER_PASSWORD__` and `__ADAPTIX_OPERATORS_BLOCK__` placeholders, points `cert`/`key` at `/app/data/server.rsa.{crt,key}`, and trims TLS cipher suites to ECDHE-only (no PFS-less RSA suites). |
| `docker/entrypoint.sh` | Runtime image's first-start bootstrap. Generates self-signed TLS cert+key into `/app/data/` if missing, then renders `/app/profile.yaml.tmpl` → `/app/data/profile.yaml` substituting `ADAPTIX_TEAMSERVER_PASSWORD` and `ADAPTIX_OPERATORS` (env vars or `openssl rand` defaults), and persists the resolved credentials to `/app/data/credentials.txt` (mode 600). Subsequent starts reuse the rendered profile — edit `data/profile.yaml` and restart, or `rm` it to re-render. See `BLUEPRINT.md` §5.7. |
...
| `build-client-linux.sh` | Wrapper around the `build-client` compose profile. Applies `patches/adaptixclient-multiarch-build.patch` (auto-revert via `trap`) so the submodule stays clean, then drives Qt 6.9.2 + ubuntu:22.04 + linuxdeployqt + appimagetool. `--arch host|amd64|arm64` (default: host) picks target. **arm64 is currently gated in both directions** — aqtinstall publishes no Linux aarch64 Qt binaries for any version through 6.11.x. Explicit `--arch arm64` hard-errors; zero-args on an arm64 host also stops (with a hint to opt into amd64-under-QEMU via `--arch amd64`). The script never silently switches arches. All build plumbing (patch ARGs, compose env vars) is in place so lifting the gate is a small change. Output: `AdaptixClient-x86_64.AppImage` in `./AdaptixClient-dist/`. Host != target runs under QEMU. `--clean` wipes the dist dir; layer cache is preserved. |
| `install-prereqs-windows.ps1` | Windows prerequisite installer. Installs MSYS2 and Git for Windows via winget, runs the two-pass pacman update, and installs all MinGW64 packages (toolchain, Qt6, OpenSSL, CMake, Ninja) required by `AdaptixC2/AdaptixClient/build.bat`. Run once per machine from an elevated PowerShell prompt before building the Windows client. See `BLUEPRINT.md` §11. |
| `patches/` | Build-time patches against submodule trees. We don't own the upstream repos, so persistent customizations live here as unified diffs and are applied/reverted by the relevant build step. Currently: `adaptixclient-macos-bundle.patch` (macOS bundle CMake block, applied on host with auto-revert by `build-client-macos.sh`), `adaptixclient-multiarch-build.patch` (parameterizes the build-client stage so it can target amd64 or arm64; applied on host with auto-revert by `build-client-linux.sh`; defaults reproduce the original amd64 behavior), and `extension-kit-nanodump-host-strip.patch` (nanodump host-arch strip fix, applied inside the build container). |
...
| `.github/workflows/build.yml` | CI: builds the runtime image on `ubuntu-latest` (amd64) and `ubuntu-24.04-arm` (arm64) for every PR, every push to `main`, and a weekly cron (Mondays 06:17 UTC). Smoke-tests the image by waiting for the HEALTHCHECK to report healthy. Catches upstream submodule drift and Dockerfile regressions early. See `BLUEPRINT.md` §12. |
