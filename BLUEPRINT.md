...
First-start bootstrap for the runtime image. Replaces the previous inline-generated cert-only entrypoint with a file COPYed into the image at build time. ~105 lines; full file at `./docker/entrypoint.sh`.

Runs under `set -euo pipefail` with an `ERR` trap that prints the failing line number before exit, so a regression surfaces as a single legible error in CI's `docker logs` dump rather than a silent partial-render.
...
2. **TLS certs** — if `/app/data/server.rsa.crt` or `.key` is missing, generate a self-signed ECDSA P-256 cert with `openssl req -x509 -nodes -newkey ec -pkeyopt ec_paramgen_curve:P-256`, 365-day validity (down from 10 years — a long-lived self-signed key on a public-facing C2 is a liability), with `subjectAltName=DNS:localhost,IP:127.0.0.1` so modern TLS clients (which ignore CN) verify against loopback by default. `chmod 600` the key. The `.rsa.` in the filenames is historical (upstream chose it before ECDSA was an option) and kept so the profile template doesn't need rewriting. `ADAPTIX_TLS_SUBJECT` overrides the issuer DN; `ADAPTIX_TLS_SAN` overrides the SAN list (openssl format, e.g. `DNS:c2.example.com,IP:10.0.0.5`). Both files end up root-owned at this point.
3. **Profile rendering** — if `/app/data/profile.yaml` is missing:
   - **Validate** `ADAPTIX_TEAMSERVER_PASSWORD` and `ADAPTIX_OPERATORS`: if either is set-but-empty (e.g., a compose typo like `ADAPTIX_OPERATORS=`), refuse to start with a clear error. Otherwise the `:=` default substitution would silently random-generate replacements and mask the misconfiguration.
   ...
   - Parse the operators list via `IFS=',' read -ra ops_arr` (bash array iteration; no word-splitting/globbing hazards). Validate each entry: must contain `:`, and both `user` and `pass` must be non-empty. Reject malformed entries up front rather than rendering a broken profile.
   - Expand the validated operators into a YAML block (`    name: "pass"` lines) in a temp file under `/tmp` (the tmpfs). An `EXIT` trap removes the tempfile on any exit path.
   ...
   - Persist the resolved password and operator string to `/app/data/credentials.txt` (also `0600`). The container log only echoes a render-confirmation message, never the password itself — `docker logs` is captured indefinitely so leaking credentials there would survive any in-place rotation.
...
5. **Vulnerability scan (Trivy)** — `aquasecurity/trivy-action@0.36.0` scans `adaptixc2-omni:latest` for OS + library CVEs. Fails on `CRITICAL` or `HIGH` severity with `ignore-unfixed: true` — we only block on things we can act on (bump base-image digest, bump apt package). Runs after smoke so we don't scan a broken image.
